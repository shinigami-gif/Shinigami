...
class ShinigamiSocialClient(private val baseUrl: String = ShinigamiBackendConfig.baseUrl, private val http: OkHttpClient = OkHttpClient(), private val gson: Gson = Gson()) {
    ...
    suspend fun activities(token: String, page: Int = 1, perPage: Int = 20) = fetchPage(token, "/api/v1/social/activities", page, perPage, ShinigamiActivity::class.java)
    ...
    suspend fun createActivity(token: String, type: String, text: String?, mediaId: Long? = null, mediaTitle: String? = null) =
        postBody(token, "/api/v1/social/activities", mapOf("type" to type, "text" to text, "mediaId" to mediaId, "mediaTitle" to mediaTitle), ShinigamiActivity::class.java)
    suspend fun createReply(token: String, activityId: String, text: String) =
        postBody(token, "/api/v1/social/activities/" + activityId + "/replies", mapOf("text" to text), ShinigamiActivityReply::class.java)
    suspend fun deleteActivity(token: String, activityId: String) =
        delete(token, "/api/v1/social/activities/" + activityId)
    suspend fun replies(token: String, activityId: String, page: Int = 1, perPage: Int = 20) = fetchPage(token, "/api/v1/social/activities/" + activityId + "/replies", page, perPage, ShinigamiActivityReply::class.java)
    ...
    private suspend fun <T> postBody(token: String, path: String, payload: Any, type: Class<T>): T =
        request(token, Request.Builder().url(baseUrl + path).post(gson.toJson(payload).toRequestBody("application/json; charset=utf-8".toMediaType())).build(), type)
    private suspend fun delete(token: String, path: String) = withContext(Dispatchers.IO) {
        http.newCall(Request.Builder().url(baseUrl + path).header("Authorization", "Bearer " + token).delete().build()).execute().use {
            check(it.isSuccessful) { "Backend social request failed: HTTP " + it.code }
        }
    }
