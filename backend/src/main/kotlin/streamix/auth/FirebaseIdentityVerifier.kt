 ...
 *
 * The backend receives the Firebase ID token produced by the Android Firebase
 * Auth client. The Firebase Admin SDK verifies its signature, issuer,
 * audience and expiry before the UID is accepted as the external subject.
 */
class FirebaseIdentityVerifier(
    ...
    override fun verify(provider: String, credential: String): ExternalIdentity? {
        ...
        val signInProvider = firebaseSignInProvider(token)
        if (signInProvider != null && signInProvider.isBlank()) return null
